document.addEventListener("DOMContentLoaded", function () {
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(successLocation, errorLocation, {
            enableHighAccuracy: true // Tambahkan untuk meningkatkan akurasi
        });
    } else {
    ...
    function successLocation(position) {
        ...
        // Debugging untuk memeriksa koordinat pengguna
        ...
        const restaurantIcon = L.icon({
            iconUrl: 'images/icons/restaurant-icon.png',
            iconSize: [25, 41],
            iconAnchor: [12, 41],
            popupAnchor: [1, -34],
            shadowSize: [41, 41]
        ...
        fetch("data/restaurants.json")
            ...
            .then((data) => {
                ...
                console.log("Data restoran:", restaurantsData);
                ...
                restaurantsData.forEach((restaurant) => {
                    ...
                    L.marker([restaurant.lat, restaurant.lng], { icon: restaurantIcon })
                        .addTo(map)
                        .bindPopup(
                            `<b>${restaurant.name}</b><br>Jarak: ${distance.toFixed(2)} km<br>Rating: ${
                                restaurant.rating !== null ? restaurant.rating : "Tidak ada rating"
                            }<br>Promo: ${restaurant.promo}
                            <br><button class="map-button" onclick="openGoogleMaps(${restaurant.lat}, ${restaurant.lng})">Dapatkan Petunjuk Arah</button>`
                        );
                });
            });
    }
    ...
    distanceBtn.addEventListener("click", () => {
        ...
        // <img src="images/icons/ion_pin-sharp.svg" alt="Jarak" class="info-icon"> 
        ...
        if (restaurantsData.length > 0) {
            ...
            const distanceList = distances.map((item) => {
    ...
    ratingBtn.addEventListener("click", () => {
        ...
        if (restaurantsData.length > 0) {
            const ratings = restaurantsData.map((restaurant) => {
                return `<li>${restaurant.name}: ${restaurant.rating !== null ? restaurant.rating : "Tidak ada rating"}</li>`;
            }).join("");
    ...
    promoBtn.addEventListener("click", () => {
        ...
        if (restaurantsData.length > 0) {
            const promos = restaurantsData.map((restaurant) => {
